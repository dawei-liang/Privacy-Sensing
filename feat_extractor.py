...
Load processed wav, compute segment spectrogram, extract embedding features, and save as csv
"""
...
if __name__ == '__main__':
    ...
    fold = 5
    # Load wave files
    audio_root = './processed_audio/option' + str(option) + '-0.77/fold' + str(fold)
    audio_file_list = [x for x in os.listdir(audio_root) if x.endswith('.wav')]
    save_feature_dir = './esc_features/tl/option' + str(option) + '-0.77/fold' + str(fold)
   
    training_feat, test_feat = np.empty((0, 513)), np.empty((0, 513))   # 512 features + a label
    ...
    print('saving tl features and labels for training, fold: %d' % fold)            
    ...
    # save mfcc test set
    print('saving tl features and labels for test, fold: %d' % fold)
