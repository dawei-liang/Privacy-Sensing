...
option = 4
save_data_dir = './esc_features/mfcc/' + 'option' + str(option) + '/'   # dir to save mfcc features, only if extract_mfcc == True
extract_mfcc = False

new_sampling_rate=16000   # Hz
window_length_secs=0.05   # sec
...
for fold in folds:
    ...
    for target_class in target:
        ...
        # loop for each training file of that class
        for file in file_list_training:
            ...
            if not extract_mfcc:
                ...
                wav_read.write_audio_data('./processed_audio/option' + str(option) + \
                                          '-0.77/fold' + str(fold) + \
                                          '/' + str(target.index(target_class)) + '_training_' + file + '.wav', 
                                          new_sampling_rate, 
                                          re_signals)
            # whether to use mfcc or not
            if extract_mfcc:
        ...
        # loop for each test file of that class    
    ...
    if extract_mfcc:
        ...
        with open(save_data_dir + 'fold' + str(fold) + '/training.csv', 'w') as csvfile:
        ...
        # save mfcc test set
        print('saving mfcc features and labels for fold:', fold)
        with open(save_data_dir + 'fold' + str(fold) + '/test.csv', 'w') as csvfile2:
